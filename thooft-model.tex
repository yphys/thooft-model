...
From the expression above, we can see that $\hat{{\cal S}}f(\nu)$ has poles at $\nu = \mu + 2i, \mu + 4i, \cdots  $.  Pictorially the these poles occurs at the position where the strings of poles $\nu + 2i \mathbb{N}$ from $S(\nu{}' -\nu)$ collides with the poles from $f(\nu)$ at $\nu = \mu$, \yp{ as illustrated in figure ? }.

Also, from \bref{SHatContinued}, we could derive the residue of $\hat{{\cal S}}f(\nu) $ for $\nu = \mu + 2ki $ to equal to be $4\pi(-1)^{k}kr_{0}$, where $r_{0}$ is the residue of $f(\nu)$ at $\mu$.

Note that we have assumed $\Im \mu > 0$. Similar analysis could be preformed with $\Im \mu < 0$ and we get that the strings of poles of $\hat{{\cal S}}f(\nu)$ that correspond to the pole $\mu$ are $\mu -2i k $, with residues $4\pi(-1)^{k}r_{0}$.

To generalize, if $f(\nu)$ has multiple poles at $\nu = \mu_{n}$, then $\hat{{\cal S}}f(\nu)$ will generated a strings of poles $\mu_{n} + 2ki $ or $\mu_{n} -2ki$ for each pole depending if the pole is in the upper half plane or the lower half plane.

Now, we shall preform the same analysis for the integral operator $\hat{{\cal{I}}}$. We assume $f(\nu)$ has a single pole at $\nu = \mu$ with $\Im \mu > 0$ as usual. Recall that the operator $\hat{{\cal{I}}}$ is defined as 
\[
